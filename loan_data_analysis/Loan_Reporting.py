...
def dataassertionreport(dataframe , primarykey_list, main_date_key):
#         '''
#         Function does Sanitcy Check on the dataframe to look for data consistency and accuracy
#         '''
    def dupcheck(dataframe , primarykey_list):
    ...
    def invalidrecords(dataframe, main_date_key):
        today = date.today()
        invalid_df = dataframe[dataframe[main_date_key].dt.date > today]
        return invalid_df
    
    def basicstats(dataframe):
        stat = sample_data.describe().transpose()
        return stat
    
    dup_df = dupcheck(dataframe , primarykey_list)
    null_df = nullcheck(dataframe)
    invalid_df = invalidrecords(dataframe, main_date_key)
    stat = basicstats(dataframe)
...
try:
    ...
    dup_output , null_output, invalid_output, stat_df = dataassertionreport(sample_data , primarykey_list, main_date_key)
...
print(report4.plot(kind="bar"))
# ax = plt.gca()
# ax.set_ylim([0 , 150000000])
xnumbers = np.linspace(0, 150000000, 2000000)
plt.yticks(xnumbers)
plt.xticks(rotation = 0, horizontalalignment = "center")
plt.title("CURRENT_UPB by LTV Cohorts and FICO Cohorts")
